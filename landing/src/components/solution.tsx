  ...
  return (
    <section className="py-20 bg-[#f8fafc]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Grid */}
        <AnimateOnScroll animation="stagger-children" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {solutions.map((s, i) => {
            const Icon = s.icon
            return (
              <div
                key={s.title}
                className="group relative rounded-2xl border border-zinc-200 bg-white p-7 hover:border-indigo-200 hover:shadow-md hover:-translate-y-[3px] transition-[transform,border-color,box-shadow] duration-200 overflow-hidden"
              >
                {/* Background number */}
                <div className="absolute top-5 right-6 font-display font-extrabold text-5xl text-zinc-900/[0.04] select-none pointer-events-none">
                  {String(i + 1).padStart(2, '0')}
                </div>

                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-100 to-violet-100 flex items-center justify-center mb-5 group-hover:from-indigo-200 group-hover:to-violet-200 transition-[background] duration-200">
                  <Icon className="w-5 h-5 text-indigo-600" />
                </div>

                <h3 className="font-display font-bold text-zinc-900 text-base mb-2">{s.title}</h3>
